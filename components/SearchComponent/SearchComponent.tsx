...
export default class SearchComponent extends React.Component<SearchProps> {
    // Defining the default props for when none given.
    public static defaultProps: Partial<SearchProps> = {
        vacationType: 'Zomervakantie',
    };

    // Generate a webstring according to the form-element inputs and redirects to gites.nl
    onComponentClick(): void {
        ...
        const webString = 'https://www.gites.nl/vakantiehuizen/' +
                           this.props.vacationType + '?max_persons=' +
                           personAmount + '&start_date=' +
                           dateFormat(dateStart) + '&end_date=' +
                           dateFormat(dateEnd);

        // TEMPLATE-WEBSTRING: https://www.gites.nl/vakantiehuizen/wintersport?max_persons=4&start_date=20-06-2005&end_date=20-06-2012
        window.location.assign(webString);
    }

    // Render function, adds visual elements to the webpage.
    render() {
        return (
